...
const billionToMillion = (string: string) => {
  const billions = string.match(billionRegex)?.[1];
  const trillions = string.match(trillionRegex)?.[1];
  if (billions) return `$${Number(billions) * 1000}M`;
  if (trillions) return `$${Number(trillions) * 1000 * 1000}M`;
  return string;
};
const csvReadableMoney = (string: string) => {
...
for (const day in existingJSON) {
  for (const stock of existingJSON[day].open) {
    ...
    data.push({ day, time: "open", ...stock });
  }
  for (const stock of existingJSON[day].close) {
    stock.marketCap = csvReadableMoney(stock.marketCap);
    stock.revenue = csvReadableMoney(stock.revenue);
    data.push({ day, time: "close", ...stock });
  }
}
